...
export const getAllOrderList = async () => {
  try {
    const res = await axiosInstance.get(`/orders/list`).then((res) => res.data);
    return res;
  } catch (error) {
    toast.error(`Something Went Wrong! ${error}`);
  }
...
export const getClienteleById = (clientele_Id) => {
  try {
    const response = axiosInstance
      .get(`/openapi/clientele/details/${clientele_Id}`)
      .then((res) => res.data);
    console.log("respnose ", response);
    return response;
  } catch (error) {
...
export const updateBeatsById = ({ collection_id, formData }) => {
  console.log("inside blog it", collection_id);
  try {
    const response = axiosInstance
      .post(`/collections/update/${collection_id}`, {
        ...formData,
      })
      .then((res) => res.data);
    return response;
...
export const updateBlogById = (blog_id, formData) => {
  ...
  try {
    ...
    const response = axiosInstance
      .put(`/blog/update/${blog_id}`, formData, config)
      .then((res) => res);
    return response;
  } catch (error) {
    console.log("error=====>", error);
